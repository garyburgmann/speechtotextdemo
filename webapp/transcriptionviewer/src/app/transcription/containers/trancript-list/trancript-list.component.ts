...
export class TrancriptListComponent implements OnInit {
  ...
  getTranscriptions() {
    this.transcriptService.GetTranscriptions(this.details.Region, this.details.ServiceKey)
      .subscribe(
        data => {
          this.transcriptions = Object.assign([], data);
          this.detailsValid = true;
          console.log('getTranscriptions: ', data);
        },
        error => {
          this._snackbar.open(
            'Error connecting to speech to text service. Please check account details. Error message: ' + error.message, 'Dismiss',{ duration: 5000 });
            console.error(error);
            clearInterval(this.timerHandle);
            this.accountService.IsSpeechValid.next(false) 
          }
      );
  }
